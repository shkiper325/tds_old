...
def main():
    ...
    rewards = []
    for epoch_num in tqdm(range(last_epoch + 1, EPOCH_COUNT)):
        cum_reward = 0
        loss = 0
        for _ in tqdm(range(ROLLOUT_PER_EPOCH), leave=False):
            l, r = rollout_for_score(pi)
        ...
        if epoch_num % 100 == 0 or epoch_num == EPOCH_COUNT - 1:
            save(data={
                'pi' : pi.state_dict(),
                'opt' : optimizer.state_dict()
                },
                fn=os.path.join('models', str(epoch_num))
            )
